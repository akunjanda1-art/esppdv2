...
import (
	...
	"esppd.local/approval-service/internal/http"
...
func main() {
	common := config.LoadCommon("approval-service", "APPROVAL_SERVICE_PORT", 8003)
	...
	if err != nil {
		log.Warn().Err(err).Msg("NATS not available; async events disabled")
	}
	if nc != nil {
		defer nc.Drain()
	}
